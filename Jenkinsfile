pipeline {
    ...
    environment {
        DOCKER_IMAGE_NAME = 'medgm/real-estate-property-service'
    }
    ...
    stages {
        ...
        stage('Verify Project Layout') {
            steps {
                script {
                    sh '''
                        echo "Verifying property-service repository layout..."
                        pwd
                        ls -la
                        if [ ! -f pom.xml ]; then
        ...
        stage('Build Docker Image') {
            steps {
                script {
                    sh """
                        echo "Building Docker image for property-service..."
            ...
            steps {
                script {
                    sh """
                        ...
                        docker tag ${DOCKER_IMAGE_NAME}:${BUILD_NUMBER} localhost:5000/real-estate-property-service:${BUILD_NUMBER}
                        docker tag ${DOCKER_IMAGE_NAME}:${BUILD_NUMBER} localhost:5000/real-estate-property-service:latest

                        docker push localhost:5000/real-estate-property-service:${BUILD_NUMBER}
                        docker push localhost:5000/real-estate-property-service:latest

                        echo "Images in local registry:"
                        docker images | grep real-estate-property-service
                    """
                }
            }
        }
